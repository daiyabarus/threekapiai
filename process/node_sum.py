...
class SUMNodePrePost:
    ...
    def _sum_kpi_values_for_dates(self, values, start_date, end_date):
        ...
        for raw_data in self.rawdaily_data[1:]:
            ...
            if start_date <= date <= end_date:
                ...
                kpi_value = float(kpi_str) if kpi_str else 0
                sum_values.setdefault(rnc, []).append(kpi_value)

        return {rnc: sum(vals) for rnc, vals in sum_values.items()}
    ...
    def process_kpi(self):
        ...
        for rnc in set(list(pre_sum_oneday.keys()) + list(post_sum_oneday.keys())):
            one_day_calc = Diff(pre_sum_oneday.get(rnc, 0), post_sum_oneday.get(rnc, 0))

            twodays_calc = Diff(
                pre_sum_twodays.get(rnc, 0), post_sum_twodays.get(rnc, 0)
            )

            oneweek_calc = Diff(
                pre_sum_oneweek.get(rnc, 0), post_sum_oneweek.get(rnc, 0)
            )

            if baseline_dict.get(self.mockpi) == "SUFFIX":
                post_baseline = post_sum_oneweek.get(rnc, 0)
                pre_baseline = pre_sum_oneweek.get(rnc, 0)
                ...
                pre_baseline = post_sum_oneweek.get(rnc, 0)
            ...
            kpi_data = [
                rnc,
                self.mockpi,
                pre_sum_oneday.get(rnc, 0),
                post_sum_oneday.get(rnc, 0),
                one_day_calc.delta,
                one_day_calc.delta_percent,
                one_day_calc.flag5_inc,
                pre_sum_twodays.get(rnc, 0),
                post_sum_twodays.get(rnc, 0),
                twodays_calc.delta,
                twodays_calc.delta_percent,
                twodays_calc.flag5_inc,
                pre_sum_oneweek.get(rnc, 0),
                post_sum_oneweek.get(rnc, 0),
                oneweek_calc.delta,
