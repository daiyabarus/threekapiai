...
class AGGNodePrePost:
    ...
    def _extract_kpi_values(self, include_busy_hour=True):
        ...
        for raw_data in self.rawhourly_data[1:]:
            ...
            if not include_busy_hour or int(hour) in self.busyhour_data:
                ...
                if date == pre_date:
                    pre_values.setdefault(rnc, []).append(kpi_value)
                elif date == post_date:
                    post_values.setdefault(rnc, []).append(kpi_value)
    ...
    def _average_kpi_values_for_dates(self, values, start_date, end_date):
        ...
        for raw_data in self.rawdaily_data[1:]:
            ...
            if start_date <= date <= end_date:
                ...
                avg_values.setdefault(rnc, []).append(kpi_value)

        return {
            rnc: round(sum(vals) / len(vals), 2) if vals else 0
            for rnc, vals in avg_values.items()
        }
    ...
    def process_kpi(self):
        ...
        for rnc in set(list(pre_avg_oneday.keys()) + list(post_avg_oneday.keys())):
            # pre_avg = (
            #     round(sum(pre_values.get(rnc, [])) / len(pre_values.get(rnc, [])), 2)
            #     if pre_values.get(rnc)
            #     else 0
            # )
            # post_avg = (
            #     round(sum(post_values.get(rnc, [])) / len(post_values.get(rnc, [])), 2)
            #     if post_values.get(rnc)
            #     else 0
            # )
            ...
            one_day_calc = Diff(pre_avg_oneday[rnc], post_avg_oneday[rnc])

            flag_result_one_day = (
                one_day_calc.flag5_inc if flag_type == "inc" else one_day_calc.flag5_dcr
            )

            twodays_calc = Diff(pre_avg_twodays[rnc], post_avg_twodays[rnc])

            flag_result_two_days = (
                twodays_calc.flag5_inc if flag_type == "inc" else twodays_calc.flag5_dcr
            )

            oneweek_calc = Diff(pre_avg_oneweek[rnc], post_avg_oneweek[rnc])
            ...
            if baseline_dict.get(self.mockpi) == "SUFFIX":
                post_baseline = post_avg_oneweek[rnc]
                pre_baseline = pre_avg_oneweek[rnc]
                ...
                pre_baseline = post_avg_oneweek[rnc]
            ...
            kpi_data = [
                ...
                flag_result_one_day,
                pre_avg_twodays[rnc],
                post_avg_twodays[rnc],
                twodays_calc.delta,
                twodays_calc.delta_percent,
                flag_result_two_days,
                pre_avg_oneweek[rnc],
                post_avg_oneweek[rnc],
                oneweek_calc.delta,
